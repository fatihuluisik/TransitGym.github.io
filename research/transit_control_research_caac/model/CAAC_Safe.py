...
class Agent():
    def __init__(self, state_dim, name, seed=123, n_stops=22, buslist=None, H_max=900.):
        random.seed(seed)
        self.seed = seed
        self.name = name
        self.gamma = 0.9
        ...
        self.H_max = H_max  # güvenlik eşiği (saniye)
    ...
    def choose_action(self, state):
        ...
        a = self._safety_clip(a, state)
        return a

    def _safety_clip(self, a, state):
        # state = [pax onboard, forward headway h_i (normalize), backward headway h_{i+1} (normalize)]
        # Sim_Engine.py bunu min(fh_sec / 600., 2.) olarak yazıyor -> saniyeye geri çeviriyoruz
        h_i_sec = state[0, 1].item() * 600.
        if h_i_sec >= self.H_max:      # zaten çok gecikmiş bus, daha fazla tutma
